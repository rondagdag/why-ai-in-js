...
const getGenerationByLevel = (level) => {
  return generations.find((g) => g.level === level);
};
const getGenerationsReversed = () => {
  return [...generations].reverse();
};
const levelNames = generations.reduce((acc, gen) => {
  acc[gen.level] = `${gen.name} (${getLevelYearRange(gen.level)})`;
  return acc;
}, {});
function getLevelYearRange(level) {
  const ranges = {
    1: "1901-1924",
    2: "1925-1945",
    3: "1946-1964",
...
export {
  APP_CONSTANTS as A,
  generations as a,
  getGenerationsReversed as b,
  getGenerationByLevel as g,
  levelNames as l
};
